...
/* ══════════════════════════════════════════════════════════════════════════
 * BOOST TARGET TABLE  (RPM × TPS%)  —  kPa absolute
 * ══════════════════════════════════════════════════════════════════════════
 *
...
{
    /* Sensor-fault treatment: force park, don't trip cut */
    if (!isfinite(map_kpa)) {
        boost_park();
        return false;
    }

    /* Immediate hard-cut: any sample at or above MAX → fuel cut now */
    ...
    /* Persistent overshoot above OVERSHOOT_KPA */
    if (map_kpa >= G8BA_BOOST_OVERSHOOT_KPA) {
        ...
        /* Decay counter; clear cut once well below overshoot threshold */
