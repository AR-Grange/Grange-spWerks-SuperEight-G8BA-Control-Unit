 ...
 * Cam sensors  : Hall effect x 4 (B1 IN, B1 EX, B2 IN, B2 EX)
 */
...
#define G8BA_DISPLACEMENT_CC    4627u          /* cm^3                        */
...
#define G8BA_FIRING_INTERVAL    90.0f          /* degCA between events (720/8) */
...
#define G8BA_TRIGGER_TOOTH_ANGLE    (360.0f / G8BA_TRIGGER_TEETH_TOTAL)  /* 10deg */
...
#define G8BA_INJ_DEAD_TIME_US       650u       /* injector dead time (us) - placeholder, see fuel_map dead-time table */
#define G8BA_INJ_MIN_PW_US          900u       /* min pulse width - bigger injector -> lift the floor slightly */
#define G8BA_INJ_MAX_PW_US          25000u     /* maximum pulse width (us)   */
...
/*
 * SC-4: Maximum advance LOWERED from 45deg -> 35deg (boost dramatically narrows
 * the safe MBT window).  ignition_map base table has been re-derived for SC.
 */
#define G8BA_IGN_ADVANCE_MAX        35.0f      /* max advance (degBTDC)        */
#define G8BA_IGN_ADVANCE_CRANK      5.0f       /* cranking advance (degBTDC)   */
#define G8BA_IGN_ADVANCE_IDLE       12.0f      /* idle advance (degBTDC)       */
...
/*
 * SC-5: Intake max advance REDUCED from 50deg -> 35deg to limit valve overlap
 ...
 */
#define G8BA_CVVT_IN_ADVANCE_MAX    35.0f      /* intake max advance (degCA)   */
#define G8BA_CVVT_EX_RETARD_MAX     30.0f      /* exhaust max retard (degCA)   */
#define G8BA_CVVT_DEADBAND_DEG      1.5f       /* position deadband (degCA)    */
 ...
 *
 * Boost target table (RPM x TPS%) is in boost_control.c.  This file owns
...
#define G8BA_BOOST_OVER_TICKS       4u         /* 4 x 50 ms = 200 ms persistent overshoot */
