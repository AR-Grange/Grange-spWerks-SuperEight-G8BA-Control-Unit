 ...
 *
 * Architecture:
 *   - Boost target table (RPM × TPS%) — selects desired MAP under given load
...
/**
 * @brief  Look up boost target (kPa absolute) from RPM × TPS table.
 ...
 * @param  clt_c              Current coolant temperature (°C) — boost gated below 50°C
...
/** Minimum CLT (°C) below which boost is forced OFF (cold-engine protection) */
...
#define BOOST_MAP_FAULT_TICKS         3u   /* 3 × 10 ms = 30 ms */
