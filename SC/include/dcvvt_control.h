 ...
 * Control loop: PID position controller on cam phase error (degCA).
 * Cam phase is measured from cam sensor edges relative to crank TDC.
 *
 * Target positions are looked up from 2D tables (RPM x Load).
 */
...
typedef struct {
    phaser_id_t id;
    floatdeg_t  target_deg;       /* commanded cam phase (degCA)           */
 ...
 * @param  oil_temp  Oil temperature (degC)
 * @param  clt_c     Coolant temperature (degC) - CVVT disabled below 60 degC
 */
 ...
 * @param  target_deg  Desired phase in degCA
 */
void dcvvt_set_target(phaser_id_t phaser, floatdeg_t target_deg);

/**
 * @brief  Park all phasers to 0deg (home position) - call on shutdown/stall.
 ...
 *         Disabled if: CLT < 60degC, oil temp < 50degC, RPM < 500.
 */
 ...
 * @param  phaser  Phaser ID
 * @return Actual cam phase (degCA), NaN if sensor invalid
 */
