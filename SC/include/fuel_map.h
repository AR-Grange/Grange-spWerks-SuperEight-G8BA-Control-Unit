 ...
 *   Target lambda   : 0.95 cruise / 0.83 WOT under boost (richer than NA - cooling)
 *
 * --------------------------------------------------------------------------
 * Base pulse-width derivation (FUELMAP_BASE_PW_MS)  [SC re-derived]
 * --------------------------------------------------------------------------
 *   At: 100 % VE, 100 kPa MAP, 20 degC IAT, lambda=1.00, 850 cc/min injectors
 *
 *   Vcyl       = 4627 / 8 = 578.375 cc
 *   air_mass   = 578.375 x 0.001204 = 0.6965 g
 ...
 * --------------------------------------------------------------------------
 *
 *   PW = BASE_PW x (VE/100) x (MAP/MAP_ref) x CLT_corr x IAT_corr
 *                x lambda_corr   + dead_time_ms
 *
 ...
 *   CLT  : 13 points - -20 ... 100 degC
 *   IAT  : 13 points - -20 ... 100 degC  (extended for hot post-SC charge)
 */
...
#define FUELMAP_IAT_POINTS     13u            /* SC: was 11 (axis extended to 100degC) */
...
/**
 * Base pulse width (ms) at 100 % VE, 100 kPa MAP, 20 degC IAT, lambda=1.00.
 ...
 *   FUELMAP_MIN_OPEN_MS(0.7) + max_dead_time(2.2 ms) = 2.9 ms > 0.9 ms (OK)
 *   (G8BA_INJ_MIN_PW_US is 900 us in SC config)
 */
#define FUELMAP_MIN_OPEN_MS         0.7f

/**
 * Maximum pulse width cap (ms).  At redline (7500 RPM) one full 720deg
